 ...
 *
 * @author: 10270
 * description: 活动--积分兑换福袋活动
 */
@Service
public class ScoreExchangeLuckBagActivityManage implements ActivityManage {
    final static Logger logger = LoggerFactory.getLogger(ScoreExchangeLuckBagActivityManage.class);
    @Autowired
    ...
    public HttpResult createActivity(CreateActivityParam param) {
        ...
        if(amount < ActivityContant.DEFAULT_LUCKY_BAG_ACTIVITY_MIN_AMOUNT){
            logger.info("开始创建积分兑换福袋活动，福袋数量小于最小值1，请求参数:{}", JsonUtils.objectToJson(param));
        ...
        try {
            ...
            if(redisLockHelper.lock(key, CommonUtil.createUUID(), 3000L)){//给key加锁，从获取礼物列表开始锁
                /**
                 * 远程调用服务组件（ServiceFeighClient提供的接口）查询礼物数量，判断礼物数量是否足够创建积分兑换福袋活动的
                ...
                if(result == null || !CommonExceptionEnum.SUCCESS.getCode().equals(result.getCode()) || result.getData() == null){
                    logger.info("开始创建积分兑换福袋活动，远程调用，获取服务组件礼物总数量失败，请求参数:{}, result:{}", JsonUtils.objectToJson(param), JsonUtils.objectToJson(result));
                    //获取礼物总数量失败
                ...
                //判断未使用的礼物数量与创建积分兑换福袋活动所需的礼物数量的大小关系
                if(total < amount){//未使用的小于所需的，还是不能创建活动成功
                    logger.info("开始创建积分兑换福袋活动，远程调用，获取服务组件礼物总数量不足，请求参数:{}, result:{}", JsonUtils.objectToJson(param), JsonUtils.objectToJson(result));
                    return HttpResult.generateHttpResult(ActivityException.SCORE_LUCKY_BAG_ACTIVITY_SERVICE_GIFT_AMOUNT_NOT_MEET);
                }
                //第一步：创建活动
                logger.info("开始创建积分兑换福袋活动----开始创建活动，请求参数:{}", JsonUtils.objectToJson(param));
                ...
                logger.info("开始创建积分兑换福袋活动----更新服务组件礼物列表，请求参数:{}", JsonUtils.objectToJson(param));
                ...
                if(updateResult == null || !CommonExceptionEnum.SUCCESS.getCode().equals(updateResult.getCode())){
                    logger.info("开始创建积分兑换福袋活动----更新服务组件礼物列表异常，请求参数:{}", JsonUtils.objectToJson(param));
            ...
            logger.info("开始创建积分兑换福袋活动，发生Exception异常，请求参数:{}", JsonUtils.objectToJson(param));
            e.printStackTrace();
            throw e;
     ...
     * 积分兑换福袋活动表(和福袋活动一样都是lucky_packet表)、礼物对应的表service_gift都要变
     * @param amount
     */
    private List<GiftDTO> packLuckyBag(Integer amount, String activityId) {
        logger.info("开始创建积分兑换福袋活动----开始包装福袋，activityId:{},amount:{}", activityId, amount);

        //远程调用服务组件的查询礼物列表接口
        HttpResult<String> result = serviceFeighClient.queryServiceGiftList();//查询礼物列表
        if(result == null || !CommonExceptionEnum.SUCCESS.getCode().equals(result.getCode()) || result.getData() == null){
            logger.info("开始创建积分兑换福袋活动，包装福袋方法内，远程调用，获取服务组件列表失败，amount:{}, activityId:{}, result:{}", amount, activityId, JsonUtils.objectToJson(result));
            //获取礼物列表失败————抛出异常
    ...
    /**
     * 启动积分兑换福袋活动
     * handler到这里直接对具体的福袋活动进行操作——启动活动:1、修改activity表中活动状态为进行中2；2、redis修改积分兑换附带活动表lucky_bag中金币库存\金币状态
     * @param activityDTO
     * @return
     */
    @Override
    ...
    public HttpResult startActivity(ActivityDTO activityDTO) {
        logger.info("启动积分兑换福袋活动, 请求参数param:{}", JsonUtils.objectToJson(activityDTO));
        ...
        try {
            ...
            logger.info("启动积分兑换福袋活动, 发生异常, 请求参数param:{}", JsonUtils.objectToJson(activityDTO));
            jedisUtil.del(RedisContant.BBS_CLOUD_ACTIVITY_SCORE_LUCKY_BAG_LIST);//发生回滚，存在的话就会删除
            e.printStackTrace();
            throw e;
        }finally {
            redisLockHelper.releaseLock(key);//释放锁
    ...
    /**
     * 终止积分兑换福袋活动
     * handler到这里直接对具体的福袋活动进行操作——终止活动:1、修改activity表中活动状态为结束3；2、redis修改福袋表lucky_bag中状态
    ...
    public HttpResult endActivity(ActivityDTO activityDTO) {
        logger.info("终止积分兑换福袋活动, 请求参数param:{}", JsonUtils.objectToJson(activityDTO));
        //生成redis分布式锁的key：防止多个请求重复开启
        String key = RedisContant.BBS_CLOUD_LOCK_ACTIVITY + activityDTO.getId();//各个活动不一样，添加活动的id
        ...
        try {
            ...
            if(redisLockHelper.lock(key, CommonUtil.createUUID(), 3000L)){//3s过期时间
                ...
                jedisUtil.del(RedisContant.BBS_CLOUD_ACTIVITY_SCORE_LUCKY_BAG_LIST);//删除redis缓存中的关于福袋表状态的key
                //活动结束, 更新福袋表lucky_bag: NORMAL正常待领取——》INVALID失效
            ...
            logger.info("终止积分兑换福袋活动, 发生异常, 请求参数param:{}", JsonUtils.objectToJson(activityDTO));
            e.printStackTrace();
            throw e;
        }finally {
            redisLockHelper.releaseLock(key);//释放锁
